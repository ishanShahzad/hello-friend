...
describe('ProductCard Properties', () => {
  ...
  describe('Property 7: ProductCard Spin Discount Display', () => {
    // Product with spin discount
    const spinProductArbitrary = fc.record({
      _id: fc.hexaString({ minLength: 24, maxLength: 24 }),
      name: fc.string({ minLength: 1, maxLength: 100 }),
      price: fc.float({ min: Math.fround(10), max: Math.fround(1000), noNaN: true }),
      hasSpinDiscount: fc.constant(true),
      spinDiscountedPrice: fc.float({ min: Math.fround(1), max: Math.fround(500), noNaN: true }),
      stock: fc.integer({ min: 1, max: 100 }),
    });

    // Property: Spin discounted price should be less than original price
    it('should display spin discounted price when hasSpinDiscount is true', () => {
      fc.assert(
        fc.property(
          spinProductArbitrary,
          (product) => {
            expect(product.hasSpinDiscount).toBe(true);
            expect(product.spinDiscountedPrice).toBeDefined();
            expect(typeof product.spinDiscountedPrice).toBe('number');
            return true;
          }
        ),
        { numRuns: 100 }
    ...
    // Property: Display price calculation for spin discount
    it('should use spinDiscountedPrice as display price when hasSpinDiscount', () => {
      fc.assert(
        fc.property(
          spinProductArbitrary,
          (product) => {
            const displayPrice = product.hasSpinDiscount 
              ? product.spinDiscountedPrice 
              : (product.discountedPrice || product.price);
            
            expect(displayPrice).toBe(product.spinDiscountedPrice);
            return true;
          }
        ),
        { numRuns: 100 }
    ...
    // Property: Original price should be shown struck through
    it('should show original price when spin discount is active', () => {
      fc.assert(
        fc.property(
          spinProductArbitrary,
          (product) => {
            const originalDisplayPrice = product.hasSpinDiscount ? product.price : null;
            expect(originalDisplayPrice).toBe(product.price);
            return true;
          }
        ),
        { numRuns: 100 }
    ...
    // Property: Spin badge should be shown
    it('should show spin badge when hasSpinDiscount and spinResult is active', () => {
      const spinResult = { hasCheckedOut: false };
      
      fc.assert(
        fc.property(
          spinProductArbitrary,
          (product) => {
            const shouldShowSpinBadge = product.hasSpinDiscount && 
              spinResult && 
              !spinResult.hasCheckedOut;
            
            expect(shouldShowSpinBadge).toBe(true);
            return true;
          }
        ),
        { numRuns: 100 }
