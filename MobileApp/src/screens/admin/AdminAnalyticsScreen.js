...
export default function AdminAnalyticsScreen({ navigation }) {
  ...
  const summaryStats = [
    { label: 'Total Revenue', value: `$${s.totalRevenue?.toFixed(2)}`, icon: 'cash-outline', color: palette.colors.success, bg: 'rgba(16,185,129,0.12)', change: `${s.revenueChange >= 0 ? '+' : ''}${s.revenueChange}%`, up: s.revenueChange >= 0 },
    { label: 'Total Orders', value: s.totalOrders, icon: 'receipt-outline', color: palette.colors.info, bg: 'rgba(99,102,241,0.12)', change: `${s.ordersChange >= 0 ? '+' : ''}${s.ordersChange}%`, up: s.ordersChange >= 0 },
    { label: 'Total Stores', value: s.totalStores, icon: 'storefront-outline', color: palette.colors.primary, bg: 'rgba(14,165,233,0.12)', change: `${s.verifiedStores} verified`, up: true },
    ...
    <GlassBackground>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}
        ...
        <View style={styles.headerRow}>
          ...
          <View style={{flex:1}}>
            <View style={styles.tagPill}><Ionicons name="sparkles" size={12} color={palette.colors.primary} /><Text style={styles.tagText}>Platform Analytics</Text></View>
            ...
            <GlassPanel key={stat.label} variant="card" style={styles.statCard}>
              <View style={styles.statHeader}>
                ...
                <View style={[styles.changeBadge, { backgroundColor: stat.up ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)' }]}>
                  <Ionicons name={stat.up ? 'arrow-up' : 'arrow-down'} size={10} color={stat.up ? palette.colors.success : palette.colors.error} />
                  <Text style={[styles.changeText, { color: stat.up ? palette.colors.success : palette.colors.error }]}>{stat.change}</Text>
                </View>
              </View>
          ...
          <GlassPanel variant="card" style={styles.chartSection}>
            ...
            <View style={styles.statusList}>
              {analytics.statusBreakdown.map((st, i) => {
                const statusColors = [palette.colors.warning, palette.colors.info, palette.colors.primary, palette.colors.success, palette.colors.error, '#f43f5e'];
...
const buildStyles = (p) => StyleSheet.create({
  ...
  rangeBtnText: { ...typography.caption, color: p.colors.textSecondary, fontWeight: fontWeight.medium },
  ...
  statLabel: { ...typography.caption, color: p.colors.textSecondary, fontWeight: fontWeight.medium },
  ...
  chartTitle: { ...typography.bodySemibold, color: p.colors.text, marginBottom: spacing.xs },
  chartSubtitle: { ...typography.caption, color: p.colors.textSecondary },
  ...
  barLabel: { ...typography.caption, color: p.colors.textSecondary, marginTop: 4, fontSize: 8 },
  statusList: { gap: spacing.sm },
  statusRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
  statusRowLeft: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, width: 100 },
  statusDot: { width: 8, height: 8, borderRadius: 4 },
  statusName: { ...typography.caption, color: p.colors.textSecondary, textTransform: 'capitalize' },
  statusBarContainer: { flex: 1, height: 8, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.06)' },
  statusBar: { height: 8, borderRadius: 4 },
  statusValue: { ...typography.caption, fontWeight: fontWeight.bold, color: p.colors.text, width: 30, textAlign: 'right' },
  ...
  topProductRank: { ...typography.caption, fontWeight: fontWeight.bold, color: p.colors.textSecondary, width: 24 },
  topProductName: { ...typography.bodySemibold, color: p.colors.text },
  topProductMeta: { ...typography.caption, color: p.colors.textSecondary },
  ...
  errorTitle: { ...typography.h3, color: p.colors.text, marginTop: spacing.md },
  errorText: { ...typography.body, color: p.colors.textSecondary, textAlign: 'center', marginTop: spacing.sm },
