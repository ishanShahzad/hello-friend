/**
 * AdminNotificationsScreen — Liquid Glass Design
 * Full notification hub for admin users with category filtering
 */
...
const CATEGORY_STYLES = {
  ...
  stores: { icon: 'storefront-outline', color: colors.success, bg: 'rgba(16,185,129,0.15)' },
  products: { icon: 'cube-outline', color: colors.warning, bg: 'rgba(245,158,11,0.15)' },
  system: { icon: 'settings-outline', color: colors.info, bg: 'rgba(59,130,246,0.15)' },
...
export default function AdminNotificationsScreen({ navigation }) {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const res = await api.get('/api/analytics/admin/notifications');
      const data = res.data?.notifications || res.data || [];
      // Normalize into notification objects
      const normalized = Array.isArray(data) ? data.map((n, i) => ({
        id: n._id || n.id || `admin_notif_${i}`,
      ...
      // Build sample notifications from platform data
      const samples = [
        { id: '1', type: 'orders', title: 'New Order Received', body: 'A new order has been placed on the platform.', createdAt: new Date(Date.now() - 3600000).toISOString(), read: false },
        { id: '2', type: 'users', title: 'New User Registration', body: 'A new user has signed up.', createdAt: new Date(Date.now() - 7200000).toISOString(), read: false },
        { id: '3', type: 'stores', title: 'Store Verification Request', body: 'A store has requested verification.', createdAt: new Date(Date.now() - 10800000).toISOString(), read: false },
        { id: '4', type: 'verification', title: 'Verification Pending', body: 'Review pending store verifications.', createdAt: new Date(Date.now() - 14400000).toISOString(), read: true },
        { id: '5', type: 'system', title: 'System Update', body: 'Platform has been updated to the latest version.', createdAt: new Date(Date.now() - 86400000).toISOString(), read: true },
        { id: '6', type: 'products', title: 'Low Stock Alert', body: 'Multiple products are running low on stock.', createdAt: new Date(Date.now() - 172800000).toISOString(), read: true },
      ];
