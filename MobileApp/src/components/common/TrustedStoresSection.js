/**
 * TrustedStoresSection — horizontal slider of stores the current user trusts.
 * Renders nothing if the user has no trusted stores yet (or is logged out).
 */
...
import { colors, spacing, fontSize, fontWeight, glass } from '../../styles/theme';
        ...
        {stores.map((s) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={styles.card}
              activeOpacity={0.85}
              onPress={() => navigation.navigate('Store', { storeSlug: s.storeSlug || s._id })}
              accessibilityLabel={`Visit ${s.storeName}`}
            >
...
const styles = StyleSheet.create({
  ...
  card: {
    width: 96, alignItems: 'center', backgroundColor: glass.bg || 'rgba(255,255,255,0.55)',
    borderRadius: 18, borderWidth: 1, borderColor: glass.border || 'rgba(255,255,255,0.5)',
    paddingVertical: spacing.md, paddingHorizontal: spacing.sm, marginRight: spacing.xs,
  },
  logoWrap: { position: 'relative', marginBottom: 6 },
  logo: { width: 56, height: 56, borderRadius: 28, backgroundColor: glass.bgSubtle, borderWidth: 2, borderColor: 'rgba(255,255,255,0.3)' },
  logoPlaceholder: { backgroundColor: colors.primary, justifyContent: 'center', alignItems: 'center' },
  verifiedDot: { position: 'absolute', bottom: -2, right: -2, backgroundColor: '#fff', borderRadius: 999, padding: 1 },
