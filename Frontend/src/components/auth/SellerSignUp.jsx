...
import { Sparkles, Eye, EyeOff, AlertCircle, CheckCircle2, Store, Phone, MapPin, ArrowLeft, ArrowRight } from 'lucide-react';
import GlassBackground from '../common/GlassBackground';
import { motion, AnimatePresence } from 'framer-motion';

const SellerSignUp = () => {
  const [step, setStep] = useState(1); // 1: account, 2: business info, 3: OTP
  ...
  const handleStep2Submit = async (e) => {
    e.preventDefault();
    if (!sellerForm.phoneNumber || sellerForm.phoneNumber.trim().length < 10) {
      setError('Please enter a valid phone number (at least 10 digits)');
      return;
    }
    if (!sellerForm.address || sellerForm.address.trim().length < 5) {
      setError('Please enter a valid address');
      return;
    }
    if (!sellerForm.city || sellerForm.city.trim().length < 2) {
      setError('Please enter your city');
      return;
    }
    if (!sellerForm.country || sellerForm.country.trim().length < 2) {
  ...
  const stepTitles = ['Create Account', 'Business Details', 'Verify Email'];
  const stepDescriptions = ['Set up your seller credentials', 'Tell us about your business', 'Check your email for the code'];
        ...
        <div className="p-8">
            ...
            {[1, 2, 3].map(s => (
              <div key={s} className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all"
                  style={{
                    background: step >= s ? 'linear-gradient(135deg, hsl(220, 70%, 55%), hsl(260, 60%, 60%))' : 'hsl(var(--muted))',
                    color: step >= s ? 'white' : 'hsl(var(--muted-foreground))'
                  }}>
                  {step > s ? '✓' : s}
                </div>
                {s < 3 && <div className="w-8 h-0.5 rounded" style={{ background: step > s ? 'hsl(var(--primary))' : 'hsl(var(--muted))' }} />}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <form onSubmit={handleStep1Next}>
              ...
              <button type="submit"
                className="w-full mt-8 py-3 px-4 rounded-xl font-semibold glow-soft hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2"
            ...
            <form onSubmit={handleStep2Submit}>
              <div className="space-y-4">
                <div>
                  ...
                  <input type="tel" name="phoneNumber" value={sellerForm.phoneNumber} onChange={handleSellerChange}
                    className="glass-input" placeholder="+1 234 567 8900" required disabled={loading} />
                </div>
                <div>
                  <label className="flex text-sm font-medium mb-1 items-center gap-2">
                    <Store size={14} style={{ color: 'hsl(var(--primary))' }} /> Business Name (Optional)
                  </label>
                  <input type="text" name="businessName" value={sellerForm.businessName} onChange={handleSellerChange}
                    className="glass-input" placeholder="Your business or brand name" disabled={loading} maxLength={100} />
                </div>
                <div>
                  <label className="flex text-sm font-medium mb-1 items-center gap-2">
                    <MapPin size={14} style={{ color: 'hsl(var(--primary))' }} /> Address <span style={{ color: 'hsl(0, 72%, 55%)' }}>*</span>
                  </label>
                  <input type="text" name="address" value={sellerForm.address} onChange={handleSellerChange}
                    className="glass-input" placeholder="Street address" required disabled={loading} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">City <span style={{ color: 'hsl(0, 72%, 55%)' }}>*</span></label>
                    <input type="text" name="city" value={sellerForm.city} onChange={handleSellerChange}
                      className="glass-input" placeholder="Your city" required disabled={loading} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Country <span style={{ color: 'hsl(0, 72%, 55%)' }}>*</span></label>
                    <input type="text" name="country" value={sellerForm.country} onChange={handleSellerChange}
                      className="glass-input" placeholder="Your country" required disabled={loading} />
                  </div>
                </div>
              </div>
              <div className="flex gap-3 mt-8">
                <button type="button" onClick={() => { setStep(1); setError(''); }}
                  className="flex-1 py-3 px-4 rounded-xl font-semibold glass-button flex items-center justify-center gap-2"
                ...
                <button type="submit" disabled={loading}
                  ...
                  style={{ background: 'linear-gradient(135deg, hsl(220, 70%, 55%), hsl(260, 60%, 60%))', color: 'white' }}>
                  {loading ? 'Sending OTP...' : <><span>Verify Email</span> <ArrowRight size={18} /></>}
                </button>
              </div>
            </form>
          )}
          ...
          {step === 3 && (
            <form onSubmit={handleVerifyOTP}>
              ...
              <button type="button" onClick={() => { setStep(2); setOtp(''); setError(''); setSuccess(''); }}
